...
export async function generateMetadata(
  ...
  return {
    ...
    openGraph: {
      ...
      locale: "en_US",
      type: "website",
      images: [{ url: heroImage ?? "/og.jpg", width: 1200, height: 630, alt: `${title} – XIPHIAS` }],
    },
  };
}
...
export default async function CountryPage(props: { params: Promise<{ country: string }> }) {
  ...
  const minInvestments = programs.map((p) => p.minInvestment).filter((n): n is number => typeof n === "number");
  const timelines = programs.map((p) => p.timelineMonths).filter((n): n is number => typeof n === "number");
  ...
  return (
    <main className="relative container mx-auto px-4 sm:px-6 lg:px-8 pb-12 text-black">
      ...
      <div className="mt-3"><Breadcrumb /></div>

      <div className="mt-6 grid gap-8 md:grid-cols-12">
        <aside className="md:col-span-4 space-y-6">
          <SidebarStatsPanel programsCount={programs.length} timelineRange={timelineRange} />
        ...
        <div className="md:col-span-8 space-y-8">
          ...
          <div className="md:hidden"><ContactForm /></div>
        </div>
      </div>
