...
// Tailwind-only, accessible, responsive privacy policy page with strict black/white text.
// ✅ Dark & light theme supported (text is black in light, white in dark — no gray text).
// ✅ SEO-ready (metadata + canonical + JSON-LD) and UX improvements.
...
const DLRow = ({ term, children }: { term: string; children: React.ReactNode }) => (
  <div className="grid grid-cols-1 md:grid-cols-5 gap-2 py-3 border-t first:border-t-0 border-black/10 dark:border-white/20">
    ...
    <dd className="md:col-span-3 text-sm text-black dark:text-white/90">{children}</dd>
  </div>
);
...
const Accordion = ({
  ...
  <details className="group rounded-lg border border-black/10 dark:border-white/20 bg-white/80 dark:bg-black/40 px-4 py-3 backdrop-blur-sm text-black dark:text-white" open={defaultOpen}>
...
export default function PrivacyPolicyPage() {
  ...
  // JSON-LD blocks (SEO). Use absolute URLs for best results.
  const jsonLdWebPage = {
    ...
    <main className="min-h-screen bg-gradient-to-b from-white to-neutral-50 dark:from-black dark:to-neutral-900 text-black dark:text-white">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10 md:py-14">
          <div className="flex items-start justify-between gap-6">
            <div className="max-w-3xl">
              <h1 className="text-2xl md:text-4xl font-semibold tracking-tight">Privacy Policy</h1>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                ...
                <Card>
                  ...
                  <p className="mt-2 text-xs opacity-70">Use these links to change your preferences at any time.</p>
                </Card>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-9 space-y-10">
            <Section id="overview" title="Who we are">
              ...
              <dl className="mt-6">
                ...
                <DLRow term="Data Protection Officer">
                  Not applicable. Contact our Privacy Team at <a className="underline" href={`mailto:${company.privacyEmail}`}>{company.privacyEmail}</a>.
                </DLRow>
                <DLRow term="India: Grievance Officer">
                  <a className="underline" href={`mailto:${company.grievanceEmail}`}>{company.grievanceEmail}</a> (we aim to respond within statutory timelines)
                </DLRow>
              </dl>
            </Section>
            ...
            <Section id="collection" title="What we collect & why we collect it">
              ...
              <div className="mt-6 grid gap-3">
                <Accordion summary="Sensitive personal information (SPI) handling">
                  <p>
                    We do not intentionally collect SPI (e.g., government IDs) unless you provide it for compliance. Where collected, we restrict use, apply additional safeguards, and honor CPRA’s right to limit use of SPI.
                  </p>
                </Accordion>
            ...
            <Section id="cookies" title="Cookies & tracking">
              <p>
                We use essential cookies to make the site work and optional cookies/SDKs for analytics and advertising. You can change your preferences any time via <Link href="/cookies" className="underline">Manage Cookies</Link>. If you are in California, some tracking may be considered a “sale”/“sharing” for cross‑context behavioral advertising — use <Link href="/privacy/ccpa-optout" className="underline">Do Not Sell/Share</Link> to opt out.
              </p>
            ...
            <Section id="rights" title="Your privacy rights">
              ...
              <p className="text-sm mt-4">To exercise your rights, email <a href={`mailto:${company.privacyEmail}`} className="underline">{company.privacyEmail}</a>. We may request additional information to verify your identity and will respond within applicable timelines.</p>
            </Section>
            ...
            <Section id="security" title="How we secure your data">
              ...
              <p className="mt-3 text-sm opacity-80">No method of transmission or storage is 100% secure. We will notify users and/or regulators where required by law in the event of a breach.</p>
            </Section>

            <Section id="children" title="Children’s privacy">
              <p>
                Our services are not directed to children. If you believe a child provided personal data, contact us so we can take appropriate action. Where parental consent is required, we will not process a child’s data without verifiable consent.
              </p>
            </Section>
            ...
            <Section id="contact" title="Contact us">
              <div className="grid md:grid-cols-2 gap-4">
                <Card>
                  ...
                  <p className="text-sm">
                    Email: <a href={`mailto:${company.privacyEmail}`} className="underline">{company.privacyEmail}</a><br />
                ...
                <Card>
                  ...
                  <p className="text-sm">
                    Email: <a href={`mailto:${company.grievanceEmail}`} className="underline">{company.grievanceEmail}</a><br />
          ...
          {/* Mobile in-page nav (appears below content on small screens) */}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 pb-12">
        <div className="rounded-xl border border-black/10 dark:border-white/20 bg-white/80 dark:bg-black/40 p-4 text-xs backdrop-blur-sm">
          ...
          <p>
            Use <Kbd>Alt</Kbd> + <Kbd>↑</Kbd>/<Kbd>↓</Kbd> to jump between headings in many browsers, or use the right‑rail links.
          </p>
        </div>
      </div>
